...
required_by: ["[[Concept_检索增强生成]]", "[[Concept_上下文工程]]"]
...
- 工程上可用压缩、结构化笔记和子 Agent 隔离局部探索，以减少主模型窗口中的冗余与干扰；这是一组 Context Engineering 策略，不是对位置鲁棒性的直接算法修复。
...
- [2026-07-16] 深化（来源：[[raw/Anthropic Effective Context Engineering for AI Agents.html]]）：在长任务中，主窗口应保存当前目标所需的高信号信息；压缩、结构化笔记和子 Agent 可分别管理历史、跨窗口状态和局部探索上下文。
