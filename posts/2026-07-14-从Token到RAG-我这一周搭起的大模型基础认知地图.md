...
flowchart TD
    ...
    subgraph T[Transformer：重复 N 个 Block]
        H[Masked Self-Attention]
        O[MLP、残差连接与归一化]
        H --> O
    end
    G --> H
    ...
    N --> H
    H -->|每层计算并保存当前 Token 的 K/V| L[各层 KV Cache]
    L -. 下一次 decode：对应层 Self-Attention 复用本层历史 K/V .-> H
    J -- 是 --> M[输出回答]
```
...
```mermaid
flowchart TD
    A[输入文本] --> B[Tokenizer 切分并输出 Token ID]
    B --> C[例如：18342]
    C --> D[在可训练的 Embedding 表 E 中查第 18342 行]
    D --> E[初始向量 x：Embedding 表的第 18342 行]
    E --> F[注入位置信息]
    F --> G[Transformer 多层 Attention 处理上下文]
    G --> H[得到上下文化表示 h_t]
    H --> I[预测下一个 Token]
    I --> J{是否处于训练阶段？}
    J -- 是 --> K[根据预测计算 Loss]
    K --> L[反向传播]
    L --> M[更新 Embedding 表和其他模型参数]
    J -- 否 --> N[完成本次前向计算]
    M --> N
```
